...
def test_check_requirements_fails_on_stale_export():
    stale = 'printf "stale-package==0.0.1\\n" > "$ABDI_EXPORT_OUT"'
    result = _run(
        ["bash", str(CONTRACT), "check"],
        env={"ABDI_EXPORT_CMD": stale},
    )
...
def test_streamlit_startup_smoke_times_out_instead_of_hanging():
    ...
    result = _run(
        ...
        env={
            ...
            "ABDI_STREAMLIT_SMOKE_HEALTH_URL": health,
        },
        timeout=15,
    )
    combined = result.stdout + result.stderr
