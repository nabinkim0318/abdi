...
def show_visualizations(df, audit_cols):
    ...
    # Handle other categorical and numerical columns as subplots
    if other_columns:
        valid_other_cols = []
        for col in other_columns:
            if not df[col].dropna().empty:
                valid_other_cols.append(col)
        
        if valid_other_cols:
            st.markdown("#### 🔍 Other Variable Distributions")
            
            # Separate categorical and numerical columns
            categorical_cols = [col for col in valid_other_cols if is_categorical(df[col])]
            numerical_cols = [col for col in valid_other_cols if not is_categorical(df[col])]
            
            # Plot categorical columns as subplots
            if categorical_cols:
            ...
            # Plot numerical columns as subplots
            if numerical_cols:
